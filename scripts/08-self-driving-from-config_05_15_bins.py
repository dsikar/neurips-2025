...
class NVIDIANet(nn.Module):
    def __init__(self, num_outputs=15, dropout_rate=0.1):  # Set num_outputs to 15 for the classes
        ...
        self.fc4 = nn.Linear(10, num_outputs)  # Output 15 classes
    ...
    def forward(self, x):
        ...
        x = F.elu(self.fc2(x))
        x = F.elu(self.fc3(x))
        x = self.fc4(x)  # Output logits for 15 classes
...
class CarlaSteering:
    def __init__(self, config, model_path='model.pth', distances_file='self_driving_distances_05.txt'):
        self.config = config
        ...
        # Initialize model
        self.model = NVIDIANet(num_outputs=15)
    ...
    def predict_steering(self, image):
        ...
        with torch.no_grad():
            class_probs = self.model(image)  # Shape: (1, 15)

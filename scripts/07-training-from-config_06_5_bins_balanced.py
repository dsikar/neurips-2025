...
# Define the discrete steering angles globally
STEERING_ANGLES = [-0.065, -0.015, 0.0, 0.015, 0.065]
...
class NVIDIANet(nn.Module):
    def __init__(self, num_outputs=5, dropout_rate=0.1):  # Changed to 5 outputs
        ...
        self.fc4 = nn.Linear(10, num_outputs)  # Outputs 5 logits for classification
...
def train_steering_model(train_dataset, val_dataset, model_save_path, batch_size=64):
    ...
    model = NVIDIANet(num_outputs=5)  # Updated to 5 outputs
...
data_dir = "/home/daniel/git/neurips-2025/scripts/carla_dataset_640x480_06_balanced/" # segmentation, no fiducials
...
model_save_path = f'best_quantized_steering_model_5_bins_balanced_{timestamp}.pth'

...
STEERING_ANGLES = [-0.065, -0.055, -0.045, -0.035, -0.025, -0.015, -0.005, 0.0, 0.005, 0.015, 0.025, 0.035, 0.045, 0.055, 0.065]
...
class NVIDIANet(nn.Module):
    def __init__(self, num_outputs=15, dropout_rate=0.1):  # Changed to 15 outputs
        ...
        self.fc4 = nn.Linear(10, num_outputs)  # Outputs 15 logits for classification
...
def train_steering_model(train_dataset, val_dataset, model_save_path, batch_size=64):
    ...
    model = NVIDIANet(num_outputs=15)  # Updated to 15 outputs
...
model_save_path = f'best_quantized_steering_model_15_bins_balanced_{timestamp}.pth'
...
# For inference:
model = NVIDIANet(num_outputs=15)
